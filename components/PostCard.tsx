...
const PostCard = ({ post }: {post: {title: string, excerpt: string, slug: string, author: {name: string, photo: {url: string}}, featuredImage: {url: string}, createdAt: Date}}) => {
  return (
    <div className='bg-slate-700 shadow-2xl rounded-lg p-0 lg:p-8 pb-12 mb-8'>
      ...
      <div className='block lg:flex text-center items-center justify-center mb-8 w-full'>
        ...
        <div className='font-medium text-gray-400'>
          ...
          <span className=''>{moment(post.createdAt).format('DD/MM/YYYY')}</span>
        </div>
      </div>   
      <p className="text-center text-lg px-4 lg:px-20 mb-8">{post.excerpt}</p>
      <div className='text-center'>
        <Link href={`/post/${post.slug}`}>
          <span className='transition duration-500 transform hover:-translate-y-1 inline-block bg-slate-500 rounded-full px-4 py-2 cursor-pointer font-medium text-lg'>Continue Reading</span>
        </Link>
      </div>
    </div>
  )
